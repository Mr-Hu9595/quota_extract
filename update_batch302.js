...
// 记录截图映射关系
const screenshotMapPath = 'E:/定额采集/screenshot_quota_map.json';
let screenshotMap = {};
try {
  screenshotMap = JSON.parse(fs.readFileSync(screenshotMapPath, 'utf8'));
} catch(e) {}

// Update 1-1-140 with data from 13-46-09-068.jpg
const quota140 = data.quotas.find(q => q.code === '1-1-140');
if (quota140) {
  quota140.comprehensive_price = 19691.85;
  quota140.labor_fee = 9127.16;
  quota140.material_fee = 1110.64;
  quota140.mechanical_fee = 2015.00;
  quota140.source_screenshot = 'bandicam 2026-04-27 13-46-09-068.jpg';
  quota140.labor_details = [
  ...
  console.log(`Updated 1-1-140: labor_fee=${quota140.labor_fee}, comprehensive_price=${quota140.comprehensive_price}`);
  screenshotMap['1-1-140'] = 'bandicam 2026-04-27 13-46-09-068.jpg';
}

// Update 1-1-143 with data from 13-47-15-547.jpg
const quota143 = data.quotas.find(q => q.code === '1-1-143');
if (quota143) {
  quota143.comprehensive_price = 31307.22;
  quota143.labor_fee = 12030.31;
  quota143.material_fee = 1437.31;
  quota143.mechanical_fee = 6576.48;
  quota143.source_screenshot = 'bandicam 2026-04-27 13-47-15-547.jpg';
  quota143.labor_details = [
  ...
  console.log(`Updated 1-1-143: labor_fee=${quota143.labor_fee}, comprehensive_price=${quota143.comprehensive_price}`);
  screenshotMap['1-1-143'] = 'bandicam 2026-04-27 13-47-15-547.jpg';
}

// Update 1-1-144 with data from 13-47-37-613.jpg
const quota144 = data.quotas.find(q => q.code === '1-1-144');
if (quota144) {
  quota144.comprehensive_price = 34680.40;
  quota144.labor_fee = 13425.91;
  quota144.material_fee = 1569.36;
  quota144.mechanical_fee = 7360.02;
  quota144.source_screenshot = 'bandicam 2026-04-27 13-47-37-613.jpg';
  quota144.labor_details = [
  ...
  quota144.mechanical_details = [
    {code:'990401035', name:'载重汽车', spec:'装载质量(t) 10', unit:'台班', quantity:0.5, quota_price:573.79, market_price:573.79, total:286.90},
    {code:'990304016', name:'汽车式起重机', spec:'提升质量(t) 16', unit:'台班', quantity:3.0, quota_price:890.11, market_price:890.11, total:2670.33},
    {code:'990304052', name:'汽车式起重机', spec:'提升质量(t) 75', unit:'台班', quantity:1.0, quota_price:3063.65, market_price:3063.65, total:3063.65}
  ];
  console.log(`Updated 1-1-144: labor_fee=${quota144.labor_fee}, comprehensive_price=${quota144.comprehensive_price}`);
  screenshotMap['1-1-144'] = 'bandicam 2026-04-27 13-47-37-613.jpg';
}
...
// Save screenshot map
fs.writeFileSync(screenshotMapPath, JSON.stringify(screenshotMap, null, 2), 'utf8');

console.log(`Total quotas now: ${data.quotas.length}`);
console.log('Saved to JSON and screenshot map');